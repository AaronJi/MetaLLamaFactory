...
from .meta_swiglu import MetaLlamaMLP,MetaSwiGLUDataDrivenActivation,MetaSwiGLUSimpleActivation
...
def get_trainable_modules(model, config: "PretrainedConfig", finetuning_args: "FinetuningArguments") -> Set[str]:
    r"""
    Freezes vision tower and language model for VLM full/freeze tuning.
    """
    trainable_modules = []
    for name, module in model.named_modules():
        if isinstance(module, MetaLlamaMLP):
            for sub_name, sub_module in module.named_modules():
                if isinstance(sub_module, MetaSwiGLUDataDrivenActivation) or isinstance(sub_module, MetaSwiGLUSimpleActivation):
                    trainable_modules.append(f"{name}.{sub_name}.beta.weight")
                    trainable_modules.append(f"{name}.{sub_name}.beta.bias")
