...
def replace_layers(model, old_layer_type, new_layer_class, config, finetuning_args):
    ...
    child_list = []
    def find_layers_to_replace(module, parent_name=''):
        for name, child in module.named_children():
            full_name = f'{parent_name}.{name}' if parent_name else name
            if isinstance(child, old_layer_type):
                layers_to_replace.append((module, name, child))
                child_list.append(child)
            else:
                find_layers_to_replace(child, full_name)

    find_layers_to_replace(model)
    print(f"len child_list:{len(child_list)}")
    for (module, name, child) in layers_to_replace:
        meta_mlp = new_layer_class(config, finetuning_args)
        
        # Copy pre-trained parameters
        meta_mlp.gate_proj.load_state_dict(child.gate_proj.state_dict())
        meta_mlp.up_proj.load_state_dict(child.up_proj.state_dict())
        meta_mlp.down_proj.load_state_dict(child.down_proj.state_dict())
    
        setattr(module, name, meta_mlp)
        # Clean up the original layer and  cache
        del child
