...
from .model_utils.visual import get_forbidden_modules, patch_target_modules, get_trainable_modules
...
def _setup_lora_tuning(
    ...
    adapter_to_resume = None #主要作用是在某些情况下恢复之前训练的LoRA适配器，以便能够继续训练，而不是从头开始训练新的适配器权重
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        if model_args.use_unsloth:
            ...
            is_mergeable = False
        # not finetuning_args.create_new_adapter 不需要创建新的适配器权重 
...
def init_adapter(
    ...
    elif finetuning_args.finetuning_type == "meta_swiglu" or finetuning_args.finetuning_type == "meta_swiglu_simple":
        _setup_meta_swiglu_tuning(model, finetuning_args, is_trainable, cast_trainable_params_to_fp32)
    else:
